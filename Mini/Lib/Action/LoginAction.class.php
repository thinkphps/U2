	...
	function login(){
		...
		if($user){
          ...
          if(!empty($cooiid)){
			...
			foreach($arrcooiid as $k=>$v){
			if(!empty($v) && $v>0){
			$cresult = $collection->field('id')->where(array('num_iid'=>$v,'uid'=>session("uniq_user_id")))->find();
			if(empty($cresult)){
			$flag = $collection->add(array('num_iid'=>$v,'uid'=>session("uniq_user_id"),'cratetime'=>$time));
			}
			}	
			}
		  }
			...
			$login_arr = array('code'=>-1,'msg'=>'请输入正确的用户名或密码');
		}
	...
	function register(){
		...
		if($res){
          ...
          if(!empty($cooiid)){
			...
			$cresult = $collection->field('id')->where(array('num_iid'=>$v,'uid'=>session("uniq_user_id")))->find();
			if(empty($cresult)){
			$collection->add(array('num_iid'=>$v,'uid'=>session("uniq_user_id"),'cratetime'=>$time));
			}
			}	
			}
		  }
