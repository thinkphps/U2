...
for($i=1;$i<=$pagenum;$i++){
//if($i==1){
$req->setFields("num_iid,title,cid,pic_url,num,list_time,delist_time,price");
$req->setPageNo($i);
$req->setIsTaobao("true");
$req->setPageSize(200);
...
$psql = "insert into `u_products` (`goods_id`,`num_iid`,`sku_id`,`properties`,`properties_name`,`quantity`,`url`) values ";
	...
	$sql = "insert into `u_goods` (`num_iid`,`catid`,`outer_id`,`title`,`num`,`price`,`pic_url`,`detail_url`,`cname`,`dname`,`props_name`,`list_time`,`delist_time`,`createtime`,`uptime`) values ('".$v['num_iid']."','".$catarr[1]['cid']."','".$product_arr['outer_id']."','".$v['title']."','".$v['num']."','".$v['price']."','".$save_image[1]."','".$product_arr['detail_url']."','".$cname."','".$catarr[1]['name']."','".$product_arr['props_name']."','".$v['list_time']."','".$v['delist_time']."','".$time."','".$time."')";
    $db->mysqlquery($sql);
