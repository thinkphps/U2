...
class GetinfoModel extends Action{
    ...
    public function getSutsValue($type){
        ...
        $sql = "select suits.`suitID`,suits.suitGenderID,suits.`suitImageUrl`,ustyle.`description`,ustyle.eglishName from  `u_suits_select` uss inner join `u_suits` suits on uss.suitID=suits.suitID inner join u_settings_suit_style ustyle on  suits.`suitStyleID`=ustyle.`ID` where suits.`approve_status`=0 and uss.`selected`='1' and  uss.`type`='".$type."'";
    ...
    public function getConSuitsList($where,$page_arr){
        ...
        $result = M('Suits')->join('left join u_settings_suit_style as g on u_suits.suitStyleID=g.ID')->field('u_suits.suitID,u_suits.suitGenderID,u_suits.suitImageUrl,g.description')->where($where)->limit($page_arr[0].','.$page_arr[1])->order('u_suits.suitID desc')->select();
        $goodsDetail = M('SuitsGoodsdetail');
        foreach($result as $k=>$v){
            $result[$k]['detail'] = $goodsDetail->join('inner join u_beubeu_goods ug on u_suits_goodsdetail.num_iid=ug.num_iid')->field('ug.num_iid,ug.pic_url,ug.detail_url,ug.title')->where(array('u_suits_goodsdetail.suitID'=>$v['suitID'],'ug.approve_status'=>'onsale','ug.num'=>array('egt','15')))->select()f;
        }
