...
class GetinfoModel extends Action{
    ...
    public function getSutsValue($type){
        ...
        $sql = "select suits.`suitID`,suits.suitGenderID,suits.`suitImageUrl`,u_suits.beubeuSuitID,ustyle.`description`,ustyle.eglishName from  `u_suits_select` uss inner join `u_suits` suits on uss.suitID=suits.suitID inner join u_settings_suit_style ustyle on  suits.`suitStyleID`=ustyle.`ID` where suits.`approve_status`=0 and uss.`selected`='1' and  uss.`type`='".$type."'";
